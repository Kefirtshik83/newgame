...
void Game::tick()
{
	...
	for (int i = 0; i < n; ++i)
	{
		if ((bowls[i].get_position().x - b_radius < Field::d || bowls[i].get_position().x + b_radius > Field::d + Field::w) && !(bowls[i].get_position().y > (Field::h - Field::hole)/2+ Field::d + Field::score_field && bowls[i].get_position().y < (Field::h - Field::hole) / 2 + Field::d + Field::score_field + Field::hole))
		{
			bowls[i].set_speed(sf::Vector2f(-(1+0.1*(bowls[i].abs_speed() < 1000))*bowls[i].get_speed().x, bowls[i].get_speed().y));
		}
		if (bowls[i].get_position().y - b_radius < Field::d+ Field::score_field || bowls[i].get_position().y + b_radius > Field::d + Field::h + Field::score_field)
		{
			bowls[i].set_speed(sf::Vector2f(bowls[i].get_speed().x, -(1 + 0.1 * (bowls[i].abs_speed() < 1000)) *bowls[i].get_speed().y));
		}
		if (bowls[i].get_position().x < -b_radius || bowls[i].get_position().x > 2 * Field::d + Field::w + b_radius)
		{
